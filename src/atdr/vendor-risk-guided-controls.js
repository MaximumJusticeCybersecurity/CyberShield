...
import './candidate-action-tournament.js';
...
function installMobilePolishStyles() {
  ...
  style.textContent = `
    ...
    @media(max-width:960px){
      body{font-size:15.5px}.topbar{position:static;padding:12px 14px;gap:10px}.brand{align-items:flex-start}.brand img{width:40px;height:40px}.brand strong{font-size:1rem}.eyebrow{font-size:.68rem;letter-spacing:.06em}
      ...
      input,select,button{min-height:44px}.actions{display:grid;grid-template-columns:1fr;gap:8px}.actions button{width:100%}.grid2,.grid3{grid-template-columns:1fr!important;gap:9px}.table-wrap{max-width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch}table{min-width:640px}th,td{padding:8px;font-size:.86rem}
    }
    @media(max-width:430px){.stepper{grid-template-columns:repeat(2,1fr)!important}.layout{width:calc(100vw - 14px)}.panel{padding:13px}table{min-width:580px}.tag{font-size:.76rem}}
  `;
...
function renderMobileStepCue() {
  ...
  const html = `<strong>Step ${stepIndex + 1} of ${STEP_LABELS.length}: ${STEP_LABELS[stepIndex]}</strong><br>Vendor-risk demo path. Keep going until the Trust Decision Record is generated.`;
...
function bootPolish(attempt = 0) {
  refreshPolish();
  if (attempt < 8 && !document.querySelector('#app')) setTimeout(() => bootPolish(attempt + 1), 150);
}
