...
const STAGES = ['Intake', 'Claims', 'Evidence', 'Gaps', 'Risk', 'Confidence', 'Review', 'Audit', 'Decision Record', 'Export'];
...
function latestHumanDecision() { return state.humanDecisions.at(-1) || null; }
function overrideStatus(decision = latestHumanDecision()) {
  if (!decision) return 'No human decision recorded';
  return decision.human_selected_action === decision.cyberShield_recommended_action ? 'No override: human confirmed CyberShield recommendation' : 'Override recorded';
}
...
function loadDemo() {
  ...
  state.overrideEvents = [];
  audit('Demo loaded', `${demo.title} loaded with synthetic evidence repository.`, '', demo.scenario_id);
  analyze();
}

function applyHumanDecisionState(record) {
  const decision = latestHumanDecision();
  record.cyberShield_recommended_action = record.recommended_action;
  record.human_decisions = [...state.humanDecisions];
  record.override_events = [...state.overrideEvents];
  record.human_selected_action = decision?.human_selected_action || '';
  record.override_status = overrideStatus(decision);
  record.human_review.decision_timestamp = decision?.decision_timestamp || '';
  record.human_review.reviewer_decision = decision?.human_selected_action || '';
  record.human_review.reviewer_notes = decision?.reviewer_notes || decision?.override_reason || '';
  record.human_review.review_status = decision ? 'Human Decision Added' : record.human_review.review_status;

  if (!decision) return record;

  if (decision.human_selected_action === 'Reject' || decision.human_selected_action === 'Quarantine') {
    record.record_defensibility_band = 'Not defensible';
    record.can_act = false;
  } else if (decision.human_selected_action === record.cyberShield_recommended_action) {
    record.record_defensibility_band = 'Executive brief ready with limitations';
    record.can_act = false;
  } else if (decision.human_selected_action === 'Accept with Caveat') {
    record.record_defensibility_band = 'Export-ready with caveat';
    record.can_act = true;
  } else if (decision.human_selected_action === 'Accept') {
    record.record_defensibility_band = 'Export-ready with caveat';
    record.can_act = true;
  } else {
    record.record_defensibility_band = 'Executive brief ready with limitations';
...
function analyze() {
  ...
  applyHumanDecisionState(state.record);
  ...
  state.record.audit_events = [...(state.record.audit_events || []), state.audit.at(-1)].filter(Boolean);
  render();
}
...
function contradictionHighlights(record = state.record) {
  ...
  const claimConflicts = record.extracted_claims
    ...
    .map(c => ({ source: c.normalized_claim, finding: c.evidence_links?.find(link => link.caveat)?.caveat || c.conflict_status, claim_id: c.claim_id }));
  const known = [];
...
function decisionBrief() {
  ...
  const humanCard = decision ? `<p><strong>Human Selected Action:</strong> ${escapeHtml(decision.human_selected_action)}</p><p><strong>Override Status:</strong> ${escapeHtml(record.override_status)}</p><p><strong>Reviewer:</strong> ${escapeHtml(decision.reviewer_name || 'Unnamed')} (${escapeHtml(decision.reviewer_role)})</p>` : '<p>No human decision recorded.</p>';
  return `
    ...
    <div class="brief-card"><span class="label">CyberShield Recommended Action</span><div class="metric warn">${escapeHtml(record.cyberShield_recommended_action)}</div></div>
    <div class="brief-card"><span class="label">Human Decision</span>${humanCard}</div>
...
function stageHeader(title, text) { return `<section class="panel"><span class="chip">${escapeHtml(title)}</span><h1>${escapeHtml(title)}</h1><p>${escapeHtml(text)}</p></section>`; }
...
function renderIntake() {
  ...
  <section class="panel"><div class="notice"><strong>Demo environment notice:</strong> This prototype demonstrates decision-assurance workflow. Do not upload real regulated, confidential, or production-sensitive evidence unless the environment has been approved for that data. Malware scanning: planned production control.</div><div class="grid2" style="margin-top:14px"><label>Demo scenario<select id="demoMode">${DEMO_MODES.map(mode => `<option value="${escapeHtml(mode.id)}" ${state.demoMode === mode.id ? 'selected' : ''}>${escapeHtml(mode.label)}</option>`).join('')}</select></label><label>Domain<select id="domain"><option value="vendor-risk" ${state.domain === 'vendor-risk' ? 'selected' : ''}>Vendor Risk</option><option value="security" ${state.domain === 'security' ? 'selected' : ''}>Security</option><option value="compliance" ${state.domain === 'compliance' ? 'selected' : ''}>Compliance</option><option value="mixed" ${state.domain === 'mixed' ? 'selected' : ''}>Mixed</option></select></label></div><div class="grid2"><label>AI source<input id="aiSource" value="${escapeHtml(state.aiSource)}"></label><label>Source model, if known<input id="sourceModel" value="${escapeHtml(state.sourceModel)}"></label></div><label>Original AI recommendation<textarea id="recommendation">${escapeHtml(state.recommendation)}</textarea></label><label>Decision context<textarea id="context">${escapeHtml(state.context)}</textarea></label><div class="grid2"><label>Intended use<input id="intendedUse" value="${escapeHtml(state.intendedUse)}"></label><label>Decision owner<input id="decisionOwner" value="${escapeHtml(state.decisionOwner)}"></label></div><div class="actions"><button id="loadDemo" class="primary" type="button">Load Vendor-Risk Demo</button><button id="analyze" class="primary" type="button">Analyze Recommendation</button></div></section>`;
}
...
function renderClaims() {
  ...
  return `${stageHeader('Claims', 'CyberShield atomizes the AI recommendation into claims that can be reviewed against evidence.')}<section class="panel"><div class="table-wrap"><table><thead><tr><th>Claim ID</th><th>Original Sentence</th><th>Normalized Claim</th><th>Type</th><th>Materiality</th><th>Required Evidence</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
}
...
function renderEvidence() {
  ...
  return `${stageHeader('Evidence', 'Synthetic evidence, local-only uploads, and evidence notes feed the same decision record. Uploaded files are not stored or transmitted by this static prototype.')}<section class="panel"><h2>Synthetic Evidence Repository</h2><label><input id="useSynthetic" type="checkbox" ${state.useSyntheticEvidence ? 'checked' : ''} style="width:auto"> Use synthetic evidence for this record</label>${synthetic}</section><section class="panel"><h2>Local In-Browser Evidence Upload</h2><div class="notice">Files are read in the browser for this session only. They are not stored by CyberShield in this static prototype.</div><label>Upload text, markdown, JSON, CSV, or readable file<input id="fileUpload" type="file" multiple></label><div class="actions"><button id="replaceEvidence" type="button">Replace Simulated Evidence</button><button id="addAlongside" type="button">Add Alongside Simulated Evidence</button></div>${uploads}</section><section class="panel"><h2>Evidence Note</h2><div class="grid2"><label>Title<input id="noteTitle" placeholder="Confidential DPA reviewed but not uploaded"></label><label>Evidence type<input id="noteType" placeholder="Data processing agreement"></label></div><label>Summary<textarea id="noteSummary" placeholder="Evidence note. Source file not uploaded."></textarea></label><label>Sufficiency caveat<input id="noteCaveat" placeholder="Reviewer summary only. Source document retained outside demo."></label><div class="actions"><button id="addNote" class="primary" type="button">Add Evidence Note</button></div>${notes}</section>`;
}
...
function renderGaps() {
  ...
  return `${stageHeader('Gaps', 'CyberShield surfaces missing support, unsupported leaps, stale evidence, self-attestation, and conflicts instead of hiding them.')}<section class="panel"><h2>Contradictory Evidence</h2><div class="table-wrap"><table><thead><tr><th>ID</th><th>Source</th><th>Severity</th><th>Finding</th></tr></thead><tbody>${contradictions}</tbody></table></div></section><section class="panel"><h2>Missing Support</h2><div class="table-wrap"><table><thead><tr><th>Claim</th><th>Missing Evidence</th><th>Severity</th><th>Finding</th></tr></thead><tbody>${missing}</tbody></table></div></section><section class="panel"><h2>AI Output Hazards</h2><div class="table-wrap"><table><thead><tr><th>ID</th><th>Hazard</th><th>Band</th></tr></thead><tbody>${hazards}</tbody></table></div></section>`;
}

function renderRisk() { return `${stageHeader('Risk If Wrong', 'Risk If Wrong focuses on consequence if the organization acts on the AI recommendation and the recommendation is wrong.')}<section class="panel"><div class="grid3"><article class="brief-card"><span class="label">Risk If Wrong</span><div class="metric bad">${escapeHtml(state.record.risk_if_wrong.band)}</div></article><article class="brief-card"><span class="label">Impact Areas</span><p>${state.record.risk_if_wrong.impact_areas.map(escapeHtml).join(', ')}</p></article><article class="brief-card"><span class="label">Action</span><p>${escapeHtml(state.record.recommended_action)}</p></article></div><div class="callout">${escapeHtml(state.record.risk_if_wrong.consequence_summary)}</div></section>`; }
function renderConfidence() { return `${stageHeader('Confidence', 'Confidence is not trust. Confidence reflects how certain CyberShield can be based on available evidence, gaps, conflicts, and human verification.')}<section class="panel"><article class="brief-card"><span class="label">Confidence Band</span><div class="metric bad">${escapeHtml(state.record.confidence_band)}</div><p>${escapeHtml(state.record.confidence_rationale)}</p></article><h2>What would raise confidence?</h2><ul><li>Current SOC 2 report with confirmed scope covering the evaluated service</li><li>Independent encryption and key-management validation</li><li>Complete subprocessor list covering AI processing path</li><li>Contractual restriction on customer data use for service improvement or model evaluation</li><li>Named vendor-risk owner, Security SME, and Legal Counsel review</li></ul></section>`; }

function renderReview() {
  const decision = latestHumanDecision();
  const decisions = state.humanDecisions.map(d => `<article class="evidence-card"><h3>${escapeHtml(d.human_selected_action)} <span class="tag ${d.override_status.includes('Override') ? 'warn' : 'good'}">${escapeHtml(d.override_status)}</span></h3><p><strong>CyberShield Recommended Action:</strong> ${escapeHtml(d.cyberShield_recommended_action)}</p><p><strong>Reviewer:</strong> ${escapeHtml(d.reviewer_name || 'Unnamed')} (${escapeHtml(d.reviewer_role)})</p><p><strong>Override reason / reviewer notes:</strong> ${escapeHtml(d.override_reason)}</p><p><strong>Residual risk acknowledgment:</strong> ${escapeHtml(d.residual_risk_acknowledgment)}</p><p><strong>Decision timestamp:</strong> ${escapeHtml(d.decision_timestamp)}</p></article>`).join('') || '<p>No human decision recorded yet.</p>';
  return `${stageHeader('Review', 'Human review is the control layer. CyberShield recommendations and human decisions are preserved separately.')}<section class="panel"><h2>CyberShield Recommendation Preserved</h2><div class="grid3"><article class="brief-card"><span class="label">CyberShield Recommended Action</span><div class="metric warn">${escapeHtml(state.record.cyberShield_recommended_action)}</div></article><article class="brief-card"><span class="label">Human Selected Action</span><div class="metric ${decision ? 'warn' : ''}">${escapeHtml(decision?.human_selected_action || 'Not recorded')}</div></article><article class="brief-card"><span class="label">Override Status</span><p>${escapeHtml(overrideStatus(decision))}</p></article></div></section><section class="panel"><h2>Human Review Gate</h2><p><strong>Required:</strong> ${state.record.human_review.required ? 'Yes' : 'No'}</p><p><strong>Required reviewer role:</strong> ${escapeHtml(state.record.human_review.required_reviewer_role)}</p><p><strong>Triggers:</strong></p><ul>${state.record.human_review.triggers.map(t => `<li>${escapeHtml(t)}</li>`).join('')}</ul></section><section class="panel"><h2>Manual Override / Human Decision</h2><div class="notice">A human decision never overwrites CyberShield's recommendation. The record preserves both.</div><div class="grid2"><label>Reviewer role<select id="reviewerRole"><option>Vendor-Risk Owner</option><option>Security SME</option><option>vCISO</option><option>Compliance Owner</option><option>Legal Counsel</option><option>Procurement Owner</option><option>Executive Sponsor</option><option>Business Owner</option><option>System Owner</option></select></label><label>Reviewer name<input id="reviewerName" placeholder="Optional"></label></div><label>Human selected action<select id="humanAction"><option>Request Evidence</option><option>Escalate for Review</option><option>Accept with Caveat</option><option>Reject</option><option>Quarantine</option><option>Accept</option></select></label><label>Override reason / reviewer notes<textarea id="overrideReason" placeholder="Explain why the human decision differs from or confirms CyberShield's recommendation."></textarea></label><label>Residual risk acknowledgment<input id="residualRisk" placeholder="Required when accepting with caveat or overriding CyberShield's recommendation."></label><div class="actions"><button id="saveDecision" class="primary" type="button">Save Human Decision</button></div>${decisions}</section>`;
}

function renderAudit() {
  const rows = [...state.audit].slice().reverse().map(a => `<tr><td>${escapeHtml(a.timestamp)}</td><td>${escapeHtml(a.event_type)}</td><td>${escapeHtml(a.event_description)}</td><td>${escapeHtml(a.before_state)}</td><td>${escapeHtml(a.after_state)}</td><td>${escapeHtml(a.reason)}</td></tr>`).join('') || '<tr><td colspan="6">No audit events recorded.</td></tr>';
  return `${stageHeader('Audit', 'CyberShield records the decision trail so later reviewers can see what happened, when, and why.')}<section class="panel"><h2>Session Audit Trail</h2><div class="table-wrap"><table><thead><tr><th>Timestamp</th><th>Event</th><th>Description</th><th>Before</th><th>After</th><th>Reason</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
}

function renderDecisionRecord() { return `${stageHeader('Decision Record', 'This is the complete structured Trust Decision Record before export.')}<section class="panel"><pre>${escapeHtml(JSON.stringify(state.record, null, 2))}</pre></section>`; }
function renderExport() { return `${stageHeader('Export', 'Export JSON now and use browser print to save a polished executive brief as PDF. DOCX is planned for the future backend build.')}<section class="panel"><h2>Available Exports</h2><div class="actions"><button id="downloadJson" class="primary" type="button">Download JSON</button><button id="printPdf" class="primary" type="button">Print / Save PDF</button></div><div class="notice" style="margin-top:14px">Browser print/PDF is the approved static prototype path. Production-grade DOCX/PDF generation should move to a real backend stack.</div></section>`; }
function renderWorkspace() { if (!state.record) return renderIntake(); return ({ Intake: renderIntake, Claims: renderClaims, Evidence: renderEvidence, Gaps: renderGaps, Risk: renderRisk, Confidence: renderConfidence, Review: renderReview, Audit: renderAudit, 'Decision Record': renderDecisionRecord, Export: renderExport }[state.stage] || renderIntake)(); }
...
function attachHandlers() {
  ...
  $('#analyze') && ($('#analyze').onclick = () => { state.recommendation = $('#recommendation').value; state.domain = $('#domain').value; state.aiSource = $('#aiSource').value; state.sourceModel = $('#sourceModel').value; state.intendedUse = $('#intendedUse').value; state.context = $('#context').value; state.decisionOwner = $('#decisionOwner').value; audit('Recommendation submitted', 'User submitted recommendation for ATDR analysis.'); analyze(); });
  ...
  $('#addNote') && ($('#addNote').onclick = () => { state.evidenceNotes.push({ title: $('#noteTitle').value || 'Evidence note', type: $('#noteType').value || 'Other', summary: $('#noteSummary').value || 'No summary provided.', caveat: $('#noteCaveat').value || 'Source file not uploaded.', timestamp: now() }); audit('Evidence note added', 'User added an evidence note without source file upload.'); analyze(); });
  $('#saveDecision') && ($('#saveDecision').onclick = () => {
    const humanAction = $('#humanAction').value;
    const cyberAction = state.record.cyberShield_recommended_action;
    const decision = {
      human_decision_id: `HD-${now().replace(/[-:TZ.]/g, '').slice(0, 14)}`,
      record_id: state.record.record_id,
      cyberShield_recommended_action: cyberAction,
      human_selected_action: humanAction,
      override_status: humanAction === cyberAction ? 'No override: human confirmed CyberShield recommendation' : 'Override recorded',
      reviewer_role: $('#reviewerRole').value,
      reviewer_name: $('#reviewerName').value,
      override_reason: $('#overrideReason').value || 'No rationale entered.',
      residual_risk_acknowledgment: $('#residualRisk').value || 'Residual risk not specified.',
      reviewer_notes: $('#overrideReason').value || 'No reviewer notes entered.',
      decision_timestamp: now()
    };
    state.humanDecisions.push(decision);
    if (decision.override_status === 'Override recorded') {
      state.overrideEvents.push({ override_event_id: `OVR-${decision.human_decision_id}`, record_id: decision.record_id, cyberShield_recommended_action: cyberAction, human_selected_action: humanAction, reviewer_role: decision.reviewer_role, reviewer_name: decision.reviewer_name, override_reason: decision.override_reason, residual_risk_acknowledgment: decision.residual_risk_acknowledgment, timestamp: decision.decision_timestamp });
    }
    audit('Human decision recorded', 'Human decision recorded and preserved separately from CyberShield recommendation.', cyberAction, humanAction, decision.override_reason);
    if (decision.override_status === 'Override recorded') audit('Manual override recorded', 'Human selected action differs from CyberShield recommendation.', cyberAction, humanAction, decision.override_reason);
    analyze();
  });
...
async function handleFiles(event) { const files = [...event.target.files]; for (const file of files) { let text = ''; try { text = await file.text(); } catch { text = `Uploaded file ${file.name} could not be read as text in this browser demo. Add an evidence note instead.`; } state.uploadedEvidence.push({ name: file.name, type: file.type || 'unknown', size: file.size, text, timestamp: now() }); audit('Evidence uploaded', `Local in-browser file added: ${file.name}.`, '', 'session-only upload'); } analyze(); }
function downloadJson() { if (!state.record) return; state.exportCount += 1; state.record.export_count = state.exportCount; state.record.last_exported_at = now(); audit('JSON exported', 'Structured ATDR JSON exported.', '', state.record.last_exported_at); state.record.audit_events = [...(state.record.audit_events || []), state.audit.at(-1)].filter(Boolean); const blob = new Blob([exportJson(state.record)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${state.record.record_id}.json`; a.click(); URL.revokeObjectURL(url); render(); }
function printPdf() { if (!state.record) return; state.exportCount += 1; state.record.export_count = state.exportCount; state.record.last_exported_at = now(); audit('PDF exported', 'Browser print/PDF executive brief generated.', '', state.record.last_exported_at); state.record.audit_events = [...(state.record.audit_events || []), state.audit.at(-1)].filter(Boolean); $('#printDoc').innerHTML = printDocument(); window.print(); render(); }
function render() { nav(); $('#workspace').innerHTML = renderWorkspace(); $('#decisionBrief').innerHTML = decisionBrief(); $('#printDoc').innerHTML = printDocument(); attachHandlers(); }
