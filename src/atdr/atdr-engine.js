...
const ANALYSIS_VERSION = 'atdr-static-deterministic-2026060414-multidomain';
...
function inferEvidenceType(text) {
  ...
  if (has(text, ['business owner', 'risk owner', 'control owner'])) return 'Human reviewer note';
  return 'Other';
}
function inferAuthority(text) { if (has(text, ['contractual', 'dpa', 'clause', 'audit'])) return 'High'; if (has(text, ['soc 2', 'report', 'scope', 'scanner', 'policy'])) return 'Medium'; if (has(text, ['self-attested', 'states', 'believes', 'business owner'])) return 'Low'; return 'Unknown'; }
function inferDate(text) { return (text.match(/\b20\d{2}\b/) || [''])[0]; }
function inferFreshness(text) { const year = Number(inferDate(text)); if (!year) return 'Unknown'; const age = new Date().getFullYear() - year; if (age <= 1) return 'Current'; if (age <= 2) return 'Aging'; return 'Stale'; }
function inferIndependence(text) { if (has(text, ['independent', 'auditor', 'contractual', 'scanner reports', 'internal audit'])) return 'Independent or system-generated'; if (has(text, ['self-attested', 'states', 'believes', 'vendor-published'])) return 'Self-attested'; return 'Unknown'; }
...
function mapEvidence(claims, evidence) {
  return claims.map(claim => {
    ...
    return { ...claim, unsupported_flag: matches.length === 0 || claim.claim_type === 'Unsupported leap', evidence_sufficiency_band: sufficiency, conflict_status: contradiction || prompt ? 'Material conflict' : stale || self || claim.claim_type === 'Unsupported leap' ? 'Minor caveat' : matches.length ? 'No conflict' : 'Unresolved conflict', evidence_links: matches.map(e => ({ evidence_id: e.evidence_id, relevance_band: 'Medium', sufficiency_band: sufficiency, caveat: caveatFor(e), conflict_flag: contradiction || prompt })) };
  });
}

function evidenceMatchesClaim(claim, evidence) {
  const c = lower(claim.normalized_claim); const e = lower(evidence.text_extract);
  if (has(c, ['soc 2'])) return e.includes('soc 2');
  if (has(c, ['evaluated service'])) return has(e, ['scope', 'evaluated ai service', 'does not name']);
  if (has(c, ['encrypt'])) return has(e, ['encrypt', 'key management']);
  if (has(c, ['customer data'])) return has(e, ['customer data', 'dpa', 'data processing', 'service improvement']);
  if (has(c, ['low risk'])) return has(e, ['business owner', 'risk', 'risk acceptance']);
  if (has(c, ['approved'])) return has(e, ['approval', 'business owner', 'contract', 'risk']);
...
function detectsContradiction(claim, matches) {
  ...
  if (has(c, ['covers the evaluated service']) && has(joined, ['does not name', 'does not clearly identify'])) return true;
  if (has(c, ['customer data access is acceptable']) && has(joined, ['service improvement', 'model evaluation', 'product analytics'])) return true;
  if (has(c, ['low risk']) && has(joined, ['no risk acceptance authority', 'self-attested', 'stale'])) return true;
  if (has(c, ['sufficient for approval']) && has(joined, ['does not clearly', 'self-attested', 'stale'])) return true;
...
function caveatFor(e) { if (e.prompt_injection_suspected) return 'Prompt-injection indicator found in evidence text.'; if (e.freshness_band === 'Stale') return 'Evidence appears stale.'; if (e.independence_band === 'Self-attested') return 'Evidence is self-attested and should not be treated as independent proof.'; return ''; }
...
function reviewGate(claims, missing, risk, confidenceResult, hazardsFound, domain) { const triggers = []; if (['High', 'Severe'].includes(risk.band)) triggers.push('Risk If Wrong is High or Severe'); if (missing.some(m => m.severity === 'High')) triggers.push('Evidence is insufficient for a material claim'); if (claims.some(c => ['Material conflict', 'Blocking conflict', 'Unresolved conflict'].includes(c.conflict_status))) triggers.push('Conflict status is unresolved, material, or blocking'); if (['Low confidence', 'Unknown confidence', 'Contradicted'].includes(confidenceResult.band)) triggers.push('Confidence is low, unknown, or contradicted'); if (hazardsFound.some(h => h.band === 'Quarantine')) triggers.push('Prompt injection or source manipulation suspected'); const role = domain === 'security' ? 'vCISO or Security SME' : domain === 'compliance' ? 'Compliance Owner and Security SME' : 'Vendor-Risk Owner, Security SME, Legal Counsel'; return { required: triggers.length > 0, triggers: triggers.length ? triggers : ['No mandatory review trigger detected by static rules.'], required_reviewer_role: role, review_status: triggers.length ? 'Review Required' : 'Not Required by Static Rules', reviewer_notes: '', reviewer_decision: '' }; }
function recommendedAction(risk, confidenceResult, review, hazardsFound, missing) { if (hazardsFound.some(h => h.band === 'Quarantine')) return { action: 'Quarantine', next_step: 'Do not rely on this recommendation until source manipulation is reviewed.' }; if (risk.band === 'Severe' || (risk.band === 'High' && confidenceResult.band === 'Low confidence')) return { action: 'Escalate for Review', next_step: 'Assign required human review before approval.' }; if (missing.length) return { action: 'Request Evidence', next_step: 'Collect required evidence for all material claims before action.' }; if (review.required) return { action: 'Escalate for Review', next_step: 'Human review is required before acceptance.' }; return { action: 'Accept with Caveat', next_step: 'Document caveats, limitations, and decision owner before use.' }; }
function defensibility(record, missing, review) { if (!record.original_ai_recommendation) return 'Incomplete'; if (record.ai_output_hazards.some(h => h.band === 'Quarantine')) return 'Not defensible'; if (missing.some(m => m.severity === 'High')) return 'Not Defensible Yet'; if (review.required && !review.reviewer_decision) return 'Executive brief ready with limitations'; return 'Export-ready with caveat'; }

export function analyzeRecommendation(input = {}) {
  const recommendation = clean(input.recommendation); const domain = classifyDomain(recommendation, input.domain || 'mixed'); const evidence = parseEvidenceText(input.evidence || ''); const claims = mapEvidence(extractClaims(recommendation, domain), evidence); const missing = missingSupport(claims); const hazardList = hazards(recommendation, claims, evidence); const risk = riskIfWrong(claims, domain, hazardList); const confidenceResult = confidence(claims, missing, risk, hazardList); const review = reviewGate(claims, missing, risk, confidenceResult, hazardList, domain); const action = recommendedAction(risk, confidenceResult, review, hazardList, missing); const now = new Date().toISOString(); const record = { record_id: id('ATDR'), record_type: 'AI Trust Decision Record', created_timestamp: now, created_by: input.createdBy || 'Demo user', original_ai_recommendation: recommendation, ai_source: input.aiSource || 'Unknown or user-pasted AI output', source_model: input.sourceModel || 'Unknown', intended_use: input.intendedUse || 'Decision support review before action', domain, recommendation_type: recommendationType(recommendation), decision_context: input.context || '', applicable_framework_references: frameworkMappings(claims, domain), extracted_claims: claims, evidence_provided: evidence, missing_support: missing, ai_output_hazards: hazardList, risk_if_wrong: risk, confidence_band: confidenceResult.band, confidence_rationale: confidenceResult.rationale, human_review: review, recommended_action: action.action, required_next_step: action.next_step, decision_owner: input.decisionOwner || '', limitations: ['Static advisory prototype. No backend persistence, tenant isolation, live model calls, live internet verification, or production integrations are active in this build.', 'AI-generated text alone is not treated as evidence for medium-impact or high-impact security, compliance, or vendor-risk recommendations.', 'Framework mappings are relevance mappings only and do not verify compliance.', 'Uploaded or pasted evidence is treated as untrusted data, not instructions.'], analysis_version: ANALYSIS_VERSION, export_version: VERSION, export_timestamp: '', audit_events: [{ event_type: 'Record created', event_description: 'Recommendation submitted to static ATDR analyzer.', timestamp: now }, { event_type: 'Claim extraction completed', event_description: `${claims.length} atomized claim(s) extracted.`, timestamp: now }, { event_type: 'Analysis completed', event_description: `${action.action}; ${confidenceResult.band}; ${risk.band} Risk If Wrong.`, timestamp: now }] };
