import { EVIDENCE_ISSUE_TAXONOMY } from './evidence-issue-taxonomy.js';
...
const VALIDATORS = [
  {
    ...
    issue_ids: ['scope_mismatch', 'stale', 'missing'],
  ...
  },
  {
    ...
    status: 'Needs Evidence',
    issue_ids: ['weak', 'self_attested', 'not_independently_verified'],
  ...
  },
  {
    ...
    status: 'Fail',
    issue_ids: ['weak', 'not_independently_verified'],
  ...
  },
  {
    ...
    status: 'Needs Evidence',
    issue_ids: ['contradictory', 'missing'],
  ...
  },
  {
    ...
    status: 'Needs Evidence',
    issue_ids: ['missing', 'scope_mismatch'],
  ...
  },
  {
    ...
    status: 'Fail',
    issue_ids: ['reviewer_authority_missing'],
...
function issue(id) {
  return EVIDENCE_ISSUE_TAXONOMY.find(item => item.id === id);
}

function issueTags(ids = []) {
  return ids.map(id => issue(id)).filter(Boolean).map(item => `<span class="tag warn" title="${escapeHtml(item.definition)}">${escapeHtml(item.label)}</span>`).join('');
}

function taxonomyList() {
  return EVIDENCE_ISSUE_TAXONOMY.map(item => `<li><strong>${escapeHtml(item.label)}:</strong> ${escapeHtml(item.definition)}</li>`).join('');
}
...
function validatorCard(v) {
  ...
  return `
    <article class="brief-card validator-card">
      ...
      <p><span class="tag ${cls}">${escapeHtml(v.status)}</span>${issueTags(v.issue_ids)}</p>
...
function installValidatorStyles() {
  ...
  style.textContent = `
    ...
    .taxonomy-list{margin:8px 0 0;padding-left:18px;color:var(--muted)}
    .taxonomy-list li{margin:4px 0;font-size:.9rem}
...
function renderValidators() {
  ...
  panel.innerHTML = `
    ...
    <details class="brief-card"><summary>Evidence issue taxonomy</summary><ul class="taxonomy-list">${taxonomyList()}</ul></details>
  `;
...
function renderSideSummary() {
  ...
  const issueCount = new Set(VALIDATORS.flatMap(v => v.issue_ids || [])).size;
  ...
  section.innerHTML = `
    ...
    <p><span class="tag bad">${failed} fail</span><span class="tag warn">${needs} need evidence</span><span class="tag warn">${issueCount} issue types</span></p>
